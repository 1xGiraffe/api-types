...
declare module "@polkadot/api-base/types/submittable" {
  interface AugmentedSubmittables<ApiType extends ApiTypes> {
    assetRegistry: {
       ...
       *
       * Asset is identified by `name` and the name must not be used to register
       * another asset.
       *
       * New asset is given `NextAssetId` - sequential asset id
       *
       * Adds mapping between `name` and assigned `asset_id` so asset id can be
       * retrieved by name too (Note: this approach is used in AMM implementation (xyk))
       *
    ...
    democracy: {
       ...
       *   proposal. Increased to Must be always greater than zero. For
       *   `FastTrackOrigin` must be equal or greater than `FastTrackVotingPeriod`.
    ...
    duster: {
      /**
       * Add account to list of non-dustable account. Account whihc are excluded
       * from udsting. If such account should be dusted - `AccountBlacklisted`
       * error is returned. Only root can perform this action.
       */
      ...
      >;
      /**
       * Dust specified account. IF account balance is < min. existential
       * deposit of given currency, and account is allowed to be dusted, the
       * remaining balance is transferred to selected account (usually treasury).
       *
      ...
      >;
      /**
       * Remove account from list of non-dustable accounts. That means account
       * can be dusted again.
       */
    ...
    elections: {
       ...
       * # <weight>
       *
       * The total number of voters and those that are defunct must be provided
       * as witness data.
       *
       * # </weight>
       */
       ...
       * and the bond of the outgoing member is slashed.
       *
       * If a runner-up is available, then the best runner-up will be removed
       * and replaces the outgoing member. Otherwise, if `rerun_election` is
       * `true`, a new phragmen election is started, else, nothing happens.
       *
       ...
       * # <weight>
       *
       * If we have a replacement, we use a small weight. Else, since this is a
       * root call and will go into phragmen, we assume full block for now.
       *
       * # </weight>
       */
       ...
       * - `origin` is a current member. In this case, the deposit is unreserved
       *   and origin is removed as a member, consequently not being a candidate
       *   for the next round anymore. Similar to
       *   [`remove_member`](Self::remove_member), if replacement runners
       *   exists, they are immediately used. If the prime is renouncing, then
       *   no prime will exist until the next round.
       *
       ...
       * [`Call::renounce_candidacy`] to get their deposit back. Losing the spot
       * in an election will always lead to a slash.
       *
       * # <weight>
       *
      ...
      >;
      /**
       * Vote for a set of candidates for the upcoming round of election. This
       * can be called to set the initial votes, or update already existing votes.
       *
       * Upon initial voting, `value` units of `who`'s balance is locked and a
       * deposit amount is reserved. The deposit is based on the number of votes
       ...
       *
       * If `value` is more than `who`'s free balance, then the maximum of the
       * two is used.
       *
       * The dispatch origin of this call must be signed.
       *
       ...
       * # <weight>
       *
       * We assume the maximum weight among all 3 cases: vote_equal, vote_more
       * and vote_less.
       *
       * # </weight>
       */
    ...
    lbp: {
       ...
       *
       * Assets to add has to match the pool assets. At least one amount has to
       * be non-zero.
       *
       ...
       *
       * Executes a swap of `asset_in` for `asset_out`. Price is determined by
       * the pool and is affected by the amount and the proportion of the pool
       * assets and the weights.
       *
       * Trading `fee` is distributed to the `fee_collector`.
       *
       * Parameters:
       *
       * - `asset_in`: The identifier of the asset being transferred from the
       *   account to the pool.
       * - `asset_out`: The identifier of the asset being transferred from the
       *   pool to the account.
       ...
       *
       * The dispatch origin for this call must be `T::CreatePoolOrigin`. The
       * pool is created with initial liquidity provided by the `pool_owner` who
       * must have sufficient funds free.
       *
       * The pool starts uninitialized and update_pool call should be called
       * once created to set the start block.
       *
       ...
       * - `initial_weight`: Initial weight of the asset_a. 1_000_000
       *   corresponding to 1% and 100_000_000 to 100% this should be higher
       *   than final weight
       * - `final_weight`: Final weight of the asset_a. 1_000_000 corresponding to
       *   1% and 100_000_000 to 100% this should be lower than initial weight
       * - `weight_curve`: The weight function used to update the LBP weights.
       *   Currently, there is only one weight function implemented, the linear function.
       ...
       * - `repay_target`: The amount of tokens to repay to separate fee_collector
       *   account. Until this amount is reached, fee will be increased to 20%
       *   and taken from the pool
       *
       * Emits `PoolCreated` event when successful.
       *
       * BEWARE: We are taking the fee from the accumulated asset. If the
       * accumulated asset is sold to the pool, the fee cost is transferred to
       * the pool. If its bought from the pool the buyer bears the cost. This
       * increases the price of the sold asset on every trade. Make sure to only
       * run this with previously illiquid assets.
       */
      ...
      >;
      /**
       * Transfer all the liquidity from a pool back to the pool owner and
       * destroy the pool. The pool data are also removed from the storage.
       *
       ...
       *
       * Executes a swap of `asset_in` for `asset_out`. Price is determined by
       * the pool and is affected by the amount and proportion of the pool
       * assets and the weights.
       *
       * Trading `fee` is distributed to the `fee_collector`.
       *
       ...
       * - `max_limit`: minimum amount of `asset_out` / amount of asset_out to be
       *   obtained from the pool in exchange for `asset_in`.
       *
    ...
    marketplace: {
      ...
      >;
      /**
       * Add royalty feature where a cut for author is provided There is
       * non-refundable reserve held for creating a royalty
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Pays a price to the current owner Transfers NFT ownership to the buyer
       ...
       * Price can be lower than current listing price Token doesn't have to be
       * currently listed
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Reverse action to make_offer Removes an offer and unreserves funds Can
       * be done by the offer maker or owner of the token
       *
       * Parameters:
       *
       * - `collection_id`: The identifier of a non-fungible token collection
    ...
    multiTransactionPayment: {
      ...
      >;
      /**
       * Remove currency from the list of supported currencies Only selected
       * members can perform this action
       *
       ...
       *
       * This allows to set a currency for an account in which all transaction
       * fees will be paid. Account balance cannot be zero.
       *
       * Chosen currency must be in the list of accepted currencies.
       *
       * When currency is set, fixed fee is withdrawn from the account to pay
    ...
    nft: {
      /**
       * Removes a token from existence. Burning needs to be enabled in the
       * permissions for the given collection type.
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Creates an NFT collection of the given collection type and sets its
       * metadata. The collection ID needs to be outside of the range of
       * reserved IDs. The creation of a collection needs to be enabled in the
       * permissions for the given collection type.
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Removes a collection from existence. Destroying of collections need to
       * be enabled in the permissions for the given collection type. Fails if
      ...
      >;
      /**
       * Mints an NFT in the specified collection and sets its metadata. Minting
       * of new items needs to be enabled in the permissions for the given
       * collection type.
       *
       * Parameters:
       *
      ...
      >;
      /**
       * Transfers NFT from account A to account B. Transfers need to be enabled
       * in the permissions for the given collection type.
       *
       * Parameters:
       *
       * - `origin`: The NFT owner
    ...
    router: {
      /**
       * Executes a buy with a series of trades specified in the route. The
       * price for each trade is determined by the corresponding AMM.
       *
       ...
       * - `route`: Series of [`Trade<AssetId>`] to be executed. A
       *   [`Trade<AssetId>`] specifies the asset pair (`asset_in`, `asset_out`)
       *   and the AMM (`pool`) in which the trade is executed.
       *
      ...
      >;
      /**
       * Executes a sell with a series of trades specified in the route. The
       * price for each trade is determined by the corresponding AMM.
       *
       * - `origin`: The executor of the trade
    ...
    tips: {
       ...
       *   This is formed as the hash of the tuple of the original tip `reason`
       *   and the beneficiary account ID.
       *
       * # <weight>
       *
       * - Complexity: `O(T)` where `T` is the number of tippers. decoding
       *   `Tipper` vec of length `T`. `T` is charged as upper bound given by
       *   `ContainsLengthBound`. The actual cost depends on the implementation
       *   of `T::Tippers`.
      ...
      >;
      /**
       * Report something `reason` that deserves a tip and claim any eventual
       * the finder's fee.
       *
       * The dispatch origin for this call must be _Signed_.
       *
       * Payment: `TipReportDepositBase` will be reserved from the origin
       * account, as well as `DataDepositPerByte` for each byte in `reason`.
       *
       ...
       *
       * The dispatch origin for this call must be _Signed_ and the tip
       * identified by `hash` must have been reported by the signing account
       * through `report_awesome` (and not through `tip_new`).
       *
       * - `hash`: The identity of the open tip for which a tip value is declared.
       *   This is formed as the hash of the tuple of the original tip `reason`
       *   and the beneficiary account ID.
       ...
       * # <weight>
       *
       * `T` is charged as upper bound given by `ContainsLengthBound`. The
       * actual cost depends on the implementation of `T::Tippers`.
       *
       * # </weight>
       */
       ...
       *
       * - `hash`: The identity of the open tip for which a tip value is declared.
       *   This is formed as the hash of the tuple of the hash of the original
       *   tip `reason` and the beneficiary account ID.
       ...
       *
       * - Complexity: `O(T)` where `T` is the number of tippers. decoding
       *   `Tipper` vec of length `T`, insert tip and check closing, `T` is
       *   charged as upper bound given by `ContainsLengthBound`. The actual
       *   cost depends on the implementation of `T::Tippers`.
       *
       ...
       * # <weight>
       *
       * - Complexity: `O(R + T)` where `R` length of `reason`, `T` is the number
       *   of tippers.
    ...
    tokens: {
       ...
       *
       * This will alter `FreeBalance` and `ReservedBalance` in storage. it will
       * also decrease the total issuance of the system (`TotalIssuance`). If
       * the new free or reserved balance is below the existential deposit, it
       * will reap the `AccountInfo`.
       *
       * The dispatch origin for this call is `root`.
       */
      setBalance: AugmentedSubmittable<
       ...
       * Transfer some liquid free balance to another account.
       *
       * `transfer` will set the `FreeBalance` of the sender and receiver. It
       * will decrease the total issuance of the system by the `TransferFee`. If
       * the sender's account is below the existential deposit as a result of
       * the transfer, the account will be reaped.
       *
       * The dispatch origin for this call must be `Signed` by the transactor.
    ...
    uniques: {
       ...
       * sender must be either the owner of the `item` or the admin of the collection.
       *
       ...
       *   the one to which permission of transfer is delegated.
       *
       ...
       *   of each member of the collection's admin team.
       *
       ...
       * unknown or in the case that the owner account does not have reservable
       * funds to pay for a deposit increase are ignored. Generally the owner
       * isn't going to call this on items whose existing deposit is less than
       * the refreshed deposit as it would only cost them, so it's of little consequence.
       *
       * It will still return an error in the case that the collection is
       * unknown of the signer is not permitted to call it.
       *
       * Weight: `O(items.len())`
       */
       ...
       *
       * - `maybe_collection`: The identifier of the collection whose ownership
       *   the signer is willing to accept, or if `None`, an indication that the
       *   signer is willing to accept no ownership transferal.
       *
       ...
       * the formula: `MetadataDepositBase + DepositPerByte * (key.len +
       * value.len)` taking into account any already reserved funds.
       *
       ...
       * to the formula: `MetadataDepositBase + DepositPerByte * data.len`
       ...
       *
       * If the origin is Signed, then funds of signer are reserved according to
       * the formula: `MetadataDepositBase + DepositPerByte * data.len` taking
       ...
       *
       * Emits `ItemPriceSet` on success if the price is not `None`. Emits
       * `ItemPriceRemoved` on success if the price is `None`.
       */
      setPrice: AugmentedSubmittable<
        (
          collection: u128 | AnyNumber | Uint8Array,
       ...
       *   `set_accept_ownership` with `collection` in order for this operation
       *   to succeed.
       *
       * Emits `OwnerChanged`.
       *
    ...
    xTokens: {
       ...
       *
       * `dest_weight_limit` is the weight for XCM execution on the dest chain,
       * and it would be charged from the transferred assets. If set below
      ...
      transfer: AugmentedSubmittable<
        ...
        [u32, u128, XcmVersionedMultiLocation, XcmV3WeightLimit]
      >;
      /**
       * Transfer `MultiAsset`.
       *
      ...
      transferWithFee: AugmentedSubmittable<
        ...
        [u32, u128, u128, XcmVersionedMultiLocation, XcmV3WeightLimit]
      >;
      /** Generic tx */
      [key: string]: SubmittableExtrinsicFunction<ApiType>;
    };
    xyk: {
       ...
       * the liquidity pool.
       *
       * `max_limit` - maximum amount of `asset_in` to be sold in exchange for
       * `asset_out`.
       *
       * Emits `BuyExecuted` when successful.
       */
       ...
       *
       * Registers new pool for given asset pair (`asset a` and `asset b`) in
       * asset registry. Asset registry creates new id or returns previously
       * created one if such pool existed before.
       *
       * Pool is created with initial liquidity provided by `origin`. Shares are
       * issued with specified initial price and represents proportion of asset
       * in the pool.
       *
       * Emits `PoolCreated` event when successful.
       */
       ...
       *
       * Emits 'LiquidityRemoved' when successful. Emits 'PoolDestroyed' when
       * pool is destroyed.
       */
      removeLiquidity: AugmentedSubmittable<
        (
    ...
    xykLiquidityMining: {
       ...
       *
       * This function calculate user rewards from liq. mining and transfer
       * rewards to `origin` account. Claiming in the same period is allowed only once.
       *
       * Parameters:
       *
       ...
       *
       * `owner` account has to have at least `total_rewards` balance. This fund
       * will be transferred from `owner` to farm account.
       *
       * The dispatch origin for this call must be `T::CreateOrigin`. !!!WARN:
       * `T::CreateOrigin` has power over funds of `owner`'s account and it
       * should be configured to trusted origin e.g Sudo or Governance.
       *
       * Parameters:
       *
       * - `origin`: global farm's owner.
       * - `total_rewards`: total rewards planned to distribute. This rewards will
       *   be distributed between all yield farms in the global farm.
       * - `planned_yielding_periods`: planned number of periods to distribute
       *   `total_rewards`. WARN: THIS IS NOT HARD DEADLINE. Not all rewards
       *   have to be distributed in `planned_yielding_periods`. Rewards are
       *   distributed based on the situation in the yield farms and can be
       *   distributed in a longer time frame but never in the shorter time frame.
       * - `blocks_per_period`: number of blocks in a single period. Min. number
       *   of blocks per period is 1.
       * - `incentivized_asset`: asset to be incentivized in XYK pools. All yield
       *   farms added into liq. mining program have to have
       *   `incentivized_asset` in their pair.
       ...
       *
       * Only XYKs with `asset_pair` with `incentivized_asset` can be added into
       * the farm. XYK pool for `asset_pair` has to exist to successfully create
       * yield farm. Yield farm for same `asset_pair` can exist only once in the
       * global farm.
       *
       * Parameters:
       *
       ...
       * - `asset_pair`: asset pair identifying yield farm. Liq. mining will be
       *   allowed for this `asset_pair` and one of the assets in the pair must
       *   be `incentivized_asset`.
       * - `multiplier`: yield farm multiplier.
       * - `loyalty_curve`: curve to calculate loyalty multiplier to distribute
       *   rewards to users with time incentive. `None` means no loyalty multiplier.
       *
       ...
       *
       * This function transfers LP shares from `origin` to pallet's account and
       * mint nft for `origin` account. Minted nft represents deposit in the liq. mining.
       *
       * Parameters:
       *
       * - `origin`: account depositing LP shares. This account has to have at
       *   least `shares_amount` of LP shares.
       ...
       *
       * This function create yield farm entry for existing deposit. LP shares
       * are not transferred and amount of LP shares is based on existing deposit.
       *
       ...
       *
       * This function resume incentivization from `GlobalFarm` and restore full
       * functionality for yield farm. Users will be able to deposit, claim and
       * withdraw again.
       *
       ...
       *
       * This function claims rewards from `GlobalFarm` last time and stops
       * yield farm incentivization from a `GlobalFarm`. Users will be able to
       * only withdraw shares(with claiming) after calling this function.
       ...
       * Only farm owner can perform this action.
       *
       * WARN: To successfully terminate a farm, farm have to be empty(all yield
       * farms in he global farm must be terminated).
       *
       * Parameters:
       *
       * - `origin`: global farm's owner.
       ...
       * Remove yield farm
       *
       * This function marks a yield farm as ready to be removed from storage
       * when it's empty. Users will be able to only withdraw shares(without
       * claiming rewards from yield farm). Unpaid rewards will be transferred
       * back to global farm and will be used to distribute to other yield farms.
       *
       * Yield farm must be stopped before calling this function.
       *
       * Only global farm's owner can perform this action. Yield farm stays in
       * the storage until it's empty(all farm entries are withdrawn). Last
       * withdrawn from yield farm trigger removing from the storage.
       *
       * Parameters:
       *
       * - `origin`: global farm's owner.
       ...
       *
       * - Yield farm is active(yield farm is not stopped) - claim and transfer
       *   rewards(if it wasn't claimed in this period) and transfer LP shares.
       * - Liq. mining is stopped - claim and transfer rewards(if it wasn't
       *   claimed in this period) and transfer LP shares.

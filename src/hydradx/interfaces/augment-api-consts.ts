...
declare module "@polkadot/api-base/types/consts" {
  interface AugmentedConsts<ApiType extends ApiTypes> {
    ...
    circuitBreaker: {
      /**
       * The maximum percentage of a pool's liquidity that can be added in a
       * block. Represented as an optional non-zero fraction (nominator,
       * denominator) with the max value being 10_000. If set to None, the
       * limits are not enforced.
       */
      defaultMaxAddLiquidityLimitPerBlock: Option<ITuple<[u32, u32]>> &
        AugmentedConst<ApiType>;
      /**
       * The maximum percentage of a pool's liquidity that can be traded in a
       * block. Represented as a non-zero fraction (nominator, denominator) with
       * the max value being 10_000.
       */
      defaultMaxNetTradeVolumeLimitPerBlock: ITuple<[u32, u32]> &
        AugmentedConst<ApiType>;
      /**
       * The maximum percentage of a pool's liquidity that can be removed in a
       * block. Represented as an optional non-zero fraction (nominator,
       * denominator) with the max value being 10_000. If set to None, the
       * limits are not enforced.
       */
    ...
    elections: {
       ...
       * Chose wisely, and consider how it will impact
       ...
       *
       * This should be sensibly high to economically ensure the pallet cannot
       * be attacked by creating a gigantic number of votes.
       */
    ...
    identity: {
      ...
      /**
       * The maximum length of a username, including its suffix and any
       * system-added delimiters.
       */
    ...
    messageQueue: {
       ...
       *
       * A good value depends on the expected message sizes, their weights, the
       * weight that is available for processing them and the maximal needed
       * message size. The maximal message size is slightly lower than this as
       * defined by [`MaxMessageLenOf`].
       */
      heapSize: u32 & AugmentedConst<ApiType>;
      /**
       * The maximum number of stale pages (i.e. of overweight messages) allowed
       * before culling can happen. Once there are more stale pages than this,
       * then historical pages may be dropped, even if they contain unprocessed
       * overweight messages.
       */
      maxStale: u32 & AugmentedConst<ApiType>;
      /**
       * The amount of weight (if any) which should be provided to the message
       * queue for servicing enqueued items.
       *
    ...
    omnipoolWarehouseLM: {
      /**
       * Maximum number of yield farms same LP shares can be re/deposited into.
       * This value always MUST BE >= 1.
       */
      maxFarmEntriesPerDeposit: u32 & AugmentedConst<ApiType>;
      /**
       * Max number of yield farms can exist in global farm. This includes all
       * farms in the storage(active, stopped, terminated).
       */
    ...
    otcSettlements: {
      /**
       * Maximum number of iterations used in the binary search algorithm to
       * find the trade amount.
       */
    ...
    referrals: {
      ...
      palletId: FrameSupportPalletId & AugmentedConst<ApiType>;
      /**
       * Registration fee details. (ID of an asset which fee is to be paid in,
       * Amount, Beneficiary account)
       */
      registrationFee: ITuple<[u32, u128, AccountId32]> &
        AugmentedConst<ApiType>;
    ...
    scheduler: {
       ...
       * NOTE:
       *
       * - Dependent pallets' benchmarks might require a higher limit for the
    ...
    staking: {
      ...
      /**
       * Weight of the actual stake in slash points calculation. Bigger the
       * value lower the calculated slash points.
       */
      ...
      /**
       * Number of periods user can't claim rewards for. User can exit but won't
       * receive any rewards. If he stay longer than `UnclaimablePeriods` he
       * will receive rewards also for these periods.
       */
    ...
    stateTrieMigration: {
       ...
       *
       * FRAME itself does not limit the key length. The concrete value must
       * therefore depend on your storage usage. A
       * [`frame_support::storage::StorageNMap`] for example can have an
       * arbitrary number of keys which are then hashed and concatenated,
       * resulting in arbitrarily long keys.
       *
       * Use the _state migration RPC_ to retrieve the length of the longest key
       * in your storage:
       * [https://github.com/paritytech/substrate/issues/11642](https://github.com/paritytech/substrate/issues/11642)
       *
       * The migration will halt with a `Halted` event if this value is too
       * small. Since there is no real penalty from over-estimating, it is
       * advised to use a large value. The default is 512 byte.
       *
       ...
       *
       * For more info see
       * [https://www.shawntabrizi.com/blog/substrate/querying-substrate-storage-via-rpc/](https://www.shawntabrizi.com/blog/substrate/querying-substrate-storage-via-rpc/)
       */
    ...
    timestamp: {
       ...
       *
       * Be aware that this is different to the _expected_ period that the block
       * production apparatus provides. Your chosen consensus system will
       * generally work with this to determine a sensible block time. For
       * example, in the Aura pallet it will be double this period on default settings.
       */
    ...
    tips: {
      /**
       * The amount held on deposit per byte within the tip report reason or
       * bounty description.
       */
       ...
       *
       * Benchmarks depend on this value, be sure to update weights file when
       * changing this value
       */
    ...
    transactionPayment: {
      /**
       * A fee multiplier for `Operational` extrinsics to compute "virtual tip"
    ...
    uniques: {
       ...
       * store in metadata, either "normal" metadata or attribute metadata.
       */
    ...
    xcmpQueue: {
       ...
       *
       * Any further channel suspensions will fail and messages may get dropped
       * without further notice. Choosing a high value (1000) is okay; the
       * trade-off that is described in [`InboundXcmpSuspended`] still applies
       * at that scale.
       */

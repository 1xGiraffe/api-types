  ...
  interface AugmentedQueries<ApiType extends ApiTypes> {
    assetRegistry: {
      ...
      assetLocations: AugmentedQuery<
        ...
        ) => Observable<Option<HydradxRuntimeXcmAssetLocation>>,
        [u32]
      > &
       ...
       * assets. This storage is used by `SufficiencyCheck`.
       */
      existentialDepositCounter: AugmentedQuery<
        ApiType,
        () => Observable<u128>,
        []
      > &
      ...
      locationAssets: AugmentedQuery<
        ...
        [HydradxRuntimeXcmAssetLocation]
      > &
        QueryableStorageEntry<ApiType, [HydradxRuntimeXcmAssetLocation]>;
      /**
    ...
    bonds: {
      ...
      bondIds: AugmentedQuery<
        ApiType,
        (
          arg:
            | ITuple<[u32, u64]>
            | [u32 | AnyNumber | Uint8Array, u64 | AnyNumber | Uint8Array]
        ) => Observable<Option<u32>>,
        [ITuple<[u32, u64]>]
      > &
        QueryableStorageEntry<ApiType, [ITuple<[u32, u64]>]>;
    ...
    circuitBreaker: {
      ...
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /**
       * Liquidity limits of assets for removing liquidity. If not set, returns
       * the default limit.
       */
      ...
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /**
       * Trade volume limits of assets set by set_trade_volume_limit. If not
       * set, returns the default limit.
       */
    ...
    collatorSelection: {
      ...
      candidacyBond: AugmentedQuery<ApiType, () => Observable<u128>, []> &
        QueryableStorageEntry<ApiType, []>;
      /**
       * The (community, limited) collation candidates. `Candidates` and
       * `Invulnerables` should be mutually exclusive.
       *
       * This list is sorted in ascending order by deposit and when the deposits
       * are equal, the least recently updated is considered greater.
       */
    ...
    dca: {
      ...
      remainingAmounts: AugmentedQuery<
        ApiType,
      ...
      scheduleIdSequencer: AugmentedQuery<ApiType, () => Observable<u32>, []> &
        QueryableStorageEntry<ApiType, []>;
      /** Keep tracking of the schedule ids to be executed in the block */
      scheduleIdsPerBlock: AugmentedQuery<
        ApiType,
      ...
      scheduleOwnership: AugmentedQuery<
        ApiType,
        (
      ...
      schedules: AugmentedQuery<
        ...
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletDcaSchedule>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Generic query */
    ...
    democracy: {
      ...
      lowestUnbaked: AugmentedQuery<ApiType, () => Observable<u32>, []> &
        QueryableStorageEntry<ApiType, []>;
      /**
       * General information concerning any proposal or referendum. The `Hash`
       * refers to the preimage of the `Preimages` provider which can be a JSON
    ...
    dynamicFees: {
      /**
       * Stores last calculated fee of an asset and block number in which it was
       * changed.. Stored as (Asset fee, Protocol fee, Block number)
       */
      assetFee: AugmentedQuery<
        ApiType,
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletDynamicFeesFeeEntry>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Generic query */
    ...
    emaOracle: {
      /**
       * Accumulator for oracle data in current block that will be recorded at
      ...
      > &
        QueryableStorageEntry<ApiType, []>;
      /**
       * Oracle storage keyed by data source, involved asset ids and the period
       * length of the oracle.
       *
       * Stores the data entry as well as the block number when the oracle was
       * first initialized.
       */
      ...
      whitelistedAssets: AugmentedQuery<
        ApiType,
        () => Observable<BTreeSet<ITuple<[U8aFixed, ITuple<[u32, u32]>]>>>,
        []
      > &
        QueryableStorageEntry<ApiType, []>;
      /** Generic query */
    ...
    evmAccounts: {
      ...
      accountExtension: AugmentedQuery<
        ApiType,
        (arg: H160 | string | Uint8Array) => Observable<Option<U8aFixed>>,
        [H160]
      > &
        QueryableStorageEntry<ApiType, [H160]>;
      /** Whitelisted addresses that are allowed to deploy smart contracts. */
      contractDeployer: AugmentedQuery<
        ApiType,
        (arg: H160 | string | Uint8Array) => Observable<Option<Null>>,
        [H160]
      > &
    ...
    identity: {
      ...
      > &
        QueryableStorageEntry<ApiType, [AccountId32]>;
      /**
       * Usernames that an authority has granted, but that the account
    ...
    lbp: {
      /**
       * Storage used for tracking existing fee collectors Not more than one fee
       * collector per asset possible
       */
    ...
    multiTransactionPayment: {
      /**
       * Curated list of currencies which fees can be paid mapped to
       * corresponding fallback price
       */
      ...
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /**
       * Asset prices from the spot price provider or the fallback price if the
       * price is not available. Updated at the beginning of every block.
       */
      ...
      > &
        QueryableStorageEntry<ApiType, [AccountId32]>;
      transactionCurrencyOverride: AugmentedQuery<
        ApiType,
        (arg: AccountId32 | string | Uint8Array) => Observable<Option<u32>>,
        [AccountId32]
      > &
    ...
    parachainSystem: {
      /**
       * Storage field that keeps track of bandwidth used by the unincluded
       * segment along with the latest HRMP watermark. Used for limiting the
       * acceptance of new blocks with respect to relay chain constraints.
       */
       ...
       * overwrite the [`:code`][sp_core::storage::well_known_keys::CODE] which
       * will result the next block process with the new validation code. This
       * concludes the upgrade process.
       */
      ...
      > &
        QueryableStorageEntry<ApiType, []>;
      /**
       * Latest included block descendants the runtime accepted. In other words,
       * these are ancestors of the currently executing block which have not
       * been included in the observed relay-chain state.
       *
       * The segment length is limited by the capacity returned from the
       * [`ConsensusHook`] configured in the pallet.
       */
    ...
    polkadotXcm: {
       ...
       *
       * Key is the blake2 256 hash of (origin, versioned `Assets`) pair. Value
       * is the number of times this pair has been trapped (usually just 1 if it
       * exists at all).
       */
    ...
    referrals: {
      /**
       * Asset fee distribution rewards information. Maps (asset_id, level) to
       * asset reward percentages.
       */
      assetRewards: AugmentedQuery<
        ApiType,
        (
          arg1: u32 | AnyNumber | Uint8Array,
          arg2:
            ...
            | number
            | Uint8Array
        ) => Observable<Option<PalletReferralsFeeDistribution>>,
        [u32, PalletReferralsLevel]
      > &
        QueryableStorageEntry<ApiType, [u32, PalletReferralsLevel]>;
      /** Counter for the related counted storage map */
      counterForPendingConversions: AugmentedQuery<
        ApiType,
        () => Observable<u32>,
        []
      > &
      ...
      > &
        QueryableStorageEntry<ApiType, [AccountId32]>;
      /**
       * Information about assets that are currently in the rewards pot. Used to
       * easily determine list of assets that need to be converted.
       */
      pendingConversions: AugmentedQuery<
        ApiType,
        (arg: u32 | AnyNumber | Uint8Array) => Observable<Option<Null>>,
        [u32]
      > &
      ...
      referralAccounts: AugmentedQuery<
        ApiType,
      ...
      > &
        QueryableStorageEntry<ApiType, [Bytes]>;
      /**
       * Referer level and total accumulated rewards over time. Maps referrer
       * account to (Level, Balance). Level indicates current rewards and
       * Balance is used to unlock next level. Dev note: we use OptionQuery here
       * because this helps to easily determine that an account if referrer account.
       */
      referrer: AugmentedQuery<
        ApiType,
        (
          arg: AccountId32 | string | Uint8Array
        ) => Observable<Option<ITuple<[PalletReferralsLevel, u128]>>>,
        [AccountId32]
      > &
        QueryableStorageEntry<ApiType, [AccountId32]>;
      /** Shares of a referral account */
      referrerShares: AugmentedQuery<
        ApiType,
      ...
      totalShares: AugmentedQuery<ApiType, () => Observable<u128>, []> &
        QueryableStorageEntry<ApiType, []>;
      /** Shares of a trader account */
      traderShares: AugmentedQuery<
        ApiType,
        (arg: AccountId32 | string | Uint8Array) => Observable<u128>,
        [AccountId32]
      > &
      ...
      routes: AugmentedQuery<
        ...
        (
          arg:
            ...
            | string
            | Uint8Array
        ) => Observable<Option<Vec<HydradxTraitsRouterTrade>>>,
        [HydradxTraitsRouterAssetPair]
      > &
        QueryableStorageEntry<ApiType, [HydradxTraitsRouterAssetPair]>;
      /** Generic query */
      [key: string]: QueryableStorageEntry<ApiType>;
    };
    scheduler: {
    ...
    stableswap: {
      ...
      assetTradability: AugmentedQuery<
        ApiType,
        (
          arg1: u32 | AnyNumber | Uint8Array,
          arg2: u32 | AnyNumber | Uint8Array
        ) => Observable<PalletStableswapTradability>,
        [u32, u32]
      > &
        QueryableStorageEntry<ApiType, [u32, u32]>;
      /** Existing pools */
      pools: AugmentedQuery<
        ...
        (
          arg: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletStableswapPoolInfo>>,
        [u32]
      > &
        QueryableStorageEntry<ApiType, [u32]>;
      /** Generic query */
    ...
    staking: {
      ...
      positions: AugmentedQuery<
        ...
        (
          arg: u128 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletStakingPosition>>,
        [u128]
      > &
        QueryableStorageEntry<ApiType, [u128]>;
      /** List of position votes. */
      positionVotes: AugmentedQuery<
        ApiType,
        (arg: u128 | AnyNumber | Uint8Array) => Observable<PalletStakingVoting>,
        [u128]
      > &
        QueryableStorageEntry<ApiType, [u128]>;
      /**
       * List of processed vote. Used to determine if the vote should be locked
       * in case of voting not in favor.
       */
      processedVotes: AugmentedQuery<
        ApiType,
        (
          arg1: AccountId32 | string | Uint8Array,
          arg2: u32 | AnyNumber | Uint8Array
        ) => Observable<Option<PalletStakingVote>>,
        [AccountId32, u32]
      > &
        QueryableStorageEntry<ApiType, [AccountId32, u32]>;
      /** Global staking state. */
      staking: AugmentedQuery<
        ApiType,
        () => Observable<PalletStakingStakingData>,
        []
      > &
        QueryableStorageEntry<ApiType, []>;
      /** Generic query */
    ...
    stateTrieMigration: {
       ...
       * Migration progress.
       *
       * This stores the snapshot of the last migrated keys. It can be set into
       * motion and move forward by any of the means provided by this pallet.
       */
    ...
    system: {
       ...
       *
       * The value has the type `(BlockNumberFor<T>, EventIndex)` because if we
    ...
    timestamp: {
       ...
       * timestamp by a node. It is then checked at the end of each block
       * execution in the `on_finalize` hook.
       */
    ...
    tips: {
       ...
       * Again, has an insecure enumerable hash since the key is guaranteed to
       * be the result of a secure hash.
       */
      ...
      > &
        QueryableStorageEntry<ApiType, [H256]>;
      /**
       * TipsMap that are not yet completed. Keyed by the hash of `(reason,
       * who)` from the value. This has the insecure enumerable hash function
    ...
    xcmpQueue: {
       ...
       *
       * This is a `StorageValue` instead of a `StorageMap` since we expect
       * multiple reads per block to different keys with a one byte payload. The
       * access to `BoundedBTreeSet` will be cached within the block and
       * therefore only included once in the proof size.
       *
       * NOTE: The PoV benchmarking cannot know this and will over-estimate, but
       * the actual proof will be smaller.
       */

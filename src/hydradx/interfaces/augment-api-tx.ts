...
declare module "@polkadot/api-base/types/submittable" {
  interface AugmentedSubmittables<ApiType extends ApiTypes> {
    ...
    dca: {
      /**
       * Creates a new DCA schedule and plans the execution in the specified
       * start execution block. If start execution block number is not
       * specified, then the schedule is planned in the consequent block.
       *
      ...
      >;
      /**
       * Admin endpoint to terminate a DCA schedule and remove it completely
       * from the chain.
       *
       * Parameters:
       *
    ...
    omnipool: {
       ...
       *
       * `add_liquidity` adds specified asset amount to pool and in exchange
       * gives the origin corresponding shares amount in form of NFT at current price.
       *
       * Asset's tradable state must contain ADD_LIQUIDITY flag, otherwise
       * `NotAllowed` error is returned.
       *
       * NFT is minted using NTFHandler which implements non-fungibles traits
       * from frame_support.
       *
       * Asset weight cap must be respected, otherwise `AssetWeightExceeded`
       * error is returned. Asset weight is ratio between new HubAsset reserve
       * and total reserve of Hub asset in Omnipool.
       *
       * Parameters:
       *
       * - `asset`: The identifier of the new asset added to the pool. Must be
       *   already in the pool
       ...
       *
       * Can be called only after pool is initialized, otherwise it returns
       * `NoStableAssetInPool`
       *
       * Initial liquidity must be transferred to pool's account for this new
       * token manually prior to calling `add_token`.
       *
       ...
       *
       * Asset's tradable states must contain SELL flag for asset_in and BUY
       * flag for asset_out, otherwise `NotAllowed` error is returned.
       *
       * Parameters:
       *
       ...
       *
       * A refund is needed when a token is refused to be added to Omnipool, and
       * initial liquidity of the asset has been already transferred to pool's account.
       *
       * Transfer is performed only when asset is not in Omnipool and pool's
       * balance has sufficient amount.
       *
       ...
       *
       * `remove_liquidity` removes specified shares amount from given
       * PositionId (NFT instance).
       *
       * Asset's tradable state must contain REMOVE_LIQUIDITY flag, otherwise
       * `NotAllowed` error is returned.
       *
    ...
    omnipoolLiquidityMining: {
      /**
       * Claim rewards from liquidity mining program for deposit represented by
       * the `deposit_id`.
       *
       * This function calculate user rewards from liquidity mining and transfer
       * rewards to `origin` account. Claiming multiple time the same period is
       * not allowed.
       *
       * Parameters:
       *
       ...
       *
       * `owner` account has to have at least `total_rewards` balance. These
       * funds will be transferred from `owner` to farm account.
       *
       ...
       * Parameters:
       *
       * - `origin`: account allowed to create new liquidity mining program(root,
       *   governance).
       * - `total_rewards`: total rewards planned to distribute. These rewards
       *   will be distributed between all yield farms in the global farm.
       ...
       *   distributed in a longer, though never in a shorter, time frame.
       * - `blocks_per_period`: number of blocks in a single period. Min. number
       *   of blocks per period is 1.
       * - `reward_currency`: payoff currency of rewards.
       * - `owner`: liq. mining farm owner. This account will be able to manage
       *   created liquidity mining program.
       * - `yield_per_period`: percentage return on `reward_currency` of all farms.
       * - `min_deposit`: minimum amount of LP shares to be deposited into the
       *   liquidity mining by each user.
       ...
       *
       * Asset with `asset_id` has to be registered in the omnipool. At most one
       * `active` yield farm can exist in one global farm for the same `asset_id`.
       *
       * Parameters:
       *
       ...
       *   for this asset and user will be able to lock LP shares into this
       *   yield farm immediately.
       ...
       *
       * This function transfers omnipool position from `origin` to pallet's
       * account and mint NFT for `origin` account. Minted NFT represents
       * deposit in the liquidity mining. User can deposit omnipool position as
       * a whole(all the LP shares in the position).
       *
       * Parameters:
       *
       ...
       * - `position_id`: id of the omnipool position to be deposited into the
       *   liquidity mining.
       *
       ...
       *
       * This function create yield farm entry for existing deposit. Amount of
       * redeposited LP shares is same as amount shares which are already
       * deposited in the deposit.
       *
       ...
       * - `global_farm_id`: id of the global farm to which user wants to
       *   redeposit LP shares.
       ...
       *
       * This function resume incentivization of the asset from the `GlobalFarm`
       * and restore full functionality or the yield farm. Users will be able to
       * deposit, claim and withdraw again.
       *
       ...
       *
       * This function claims rewards from `GlobalFarm` last time and stop yield
       * farm incentivization from a `GlobalFarm`. Users will be able to only
       * withdraw shares(with claiming) after calling this function.
       ...
       * back to global farm and it will be used to distribute to other yield farms.
       *
      ...
      >;
      /**
       * This function claim rewards and withdraw LP shares from yield farm.
       * Omnipool position is transferred to origin only if this is last
       * withdraw in the deposit and deposit is destroyed. This function claim
       * rewards only if yield farm is not terminated and user didn't already
       * claim rewards in current period.
       *
       * Unclaimable rewards represents rewards which user won't be able to
       * claim because of exiting early and these rewards will be transferred
       * back to global farm for future redistribution.
       *
       * Parameters:
       *
       * - `origin`: owner of deposit.
       ...
       * - `DepositDestroyed` event when this was last withdraw from the deposit
       *   and deposit was destroyed.
       */
    ...
    otc: {
       ...
       * - `order_id`: ID of the order
       *
       * Events: `Filled` event when successful.
       */
       ...
       * - After the partial_fill, the remaining order.amount_in must be higher
       *   than the existential deposit of asset_in multiplied by
       *   ExistentialDepositMultiplier
       * - After the partial_fill, the remaining order.amount_out must be higher
       *   than the existential deposit of asset_out multiplied by
       *   ExistentialDepositMultiplier
       *
       * Events: `PartiallyFilled` event when successful.
       */
       ...
       * - Amount_in must be higher than the existential deposit of asset_in
       *   multiplied by ExistentialDepositMultiplier
       * - Amount_out must be higher than the existential deposit of asset_out
       *   multiplied by ExistentialDepositMultiplier
       *
       * Events:
       *
